...
### 8.4 Ecosystem-wide legacy address migration requirements

Migration scope MUST cover every Thronos surface that references legacy THR addresses, including:
- EVN / internal ecosystem identity records
...
- external app account bindings

Core migration authority rule:
- legacy addresses may remain visible for historical reads
- future write authority MUST move to Wallet V1 secp256k1-derived address
...
- recovery metadata
- audit trail linking `old_address -> new_v1_address`

Migration MUST NOT expose or store:
- private keys
- mnemonics
- seeds
- old send secrets
...
Post-migration required state:
- Wallet V1 address becomes write/signing authority
- legacy address becomes historical/read-only
- legacy address cannot initiate new writes
- historical records remain visible and linked
...
### 9.1 Migration bind/authority kinds (required)

To support controlled migration across all services, add canonical bind kinds:
- `wallet_migration_bind`
- `wallet_migration_confirm`
- `wallet_migration_revoke_legacy_write`

These kinds MUST produce immutable audit events proving:
- proof-of-legacy ownership at migration time
- proof-of-new Wallet V1 ownership (signature from new address)
- authority cutover timestamp
- linked scope (all services vs service-specific binding)
...
Ecosystem migration tests (required):
1. migrated address can execute new Wallet V1 writes
2. legacy address is read-only after cutover
3. balances/assets unchanged pre/post migration
4. NFT/pool/mining ownership continuity preserved
5. pledge/whitelist admission continuity preserved
6. external app bindings resolve to new authority
7. no secret material persists in migration artifacts
