...
def health_check():
    ...
    Returns:
        JSON with health status and system information
    """
    try:
        # Basic health info
        health_data = {
            ...
            "uptime_seconds": time.time() - health_bp.start_time if hasattr(health_bp, 'start_time') else 0
        }
        ...
        critical_missing = [
            name for name, exists in health_data["data_files"].items()
            if not exists
        ]
        
        if critical_missing:
            health_data["status"] = "degraded"
            health_data["warnings"] = [f"Missing critical file: {name}" for name in critical_missing]
...
def stats_summary():
    ...
    Returns:
        JSON with key blockchain statistics
    """
    try:
        from server import load_json, LEDGER_FILE, CHAIN_FILE, PLEDGE_CHAIN, HEIGHT_OFFSET
        ...
        return jsonify({
            "error": f"Failed to get stats summary: {str(e)}"
        }), 500
