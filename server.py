...
def api_v1_create_course():
    """
    Create a new course with optional embedded materials and quiz metadata.

    Accepts JSON or multipart form-data (for PDF slides). The expected fields
    are:

    - title: course name
    - description: optional description
    - teacher: THR address of the instructor
    - price_thr: cost in THR tokens
    - reward_l2e: L2E reward on completion
    - auth_secret/passphrase: teacher authentication
    - content_type: pdf | video | external
        * pdf: include file field "slides" (PDF)
        * video: include "video_url"
        * external: include "content_url"
    - quiz: JSON payload with ``pass_score`` and ``questions`` (or individual
      fields pass_score + quiz_questions JSON)
    """
    ...
    if isinstance(quiz_payload, str):
        try:
            quiz_payload = json.loads(quiz_payload)
        except Exception:
            quiz_payload = None
    quiz_questions_raw = data.get("quiz_questions")
    if isinstance(quiz_questions_raw, str):
        try:
            quiz_questions_raw = json.loads(quiz_questions_raw)
        except Exception:
            quiz_questions_raw = None

    def build_quiz(payload, questions_from_field):
        if not payload and not questions_from_field:
            return None
        quiz_obj = payload if isinstance(payload, dict) else {}
        pass_score_raw = quiz_obj.get("pass_score") if quiz_obj else data.get("pass_score")
        try:
            pass_score = int(pass_score_raw) if pass_score_raw is not None else 0
        except Exception:
            pass_score = 0
        raw_questions = quiz_obj.get("questions") if quiz_obj else None
        if raw_questions is None:
            raw_questions = questions_from_field if isinstance(questions_from_field, list) else []
    ...
    quiz = build_quiz(quiz_payload, quiz_questions_raw)
    ...
    materials = {"type": content_type or "external"}
    if materials["type"] == "pdf":
        slides_file = request.files.get("slides") if is_multipart else None
        if not slides_file or not slides_file.filename:
            return jsonify(status="error", message="Slides PDF required"), 400
        ext = os.path.splitext(secure_filename(slides_file.filename))[1].lower()
        if ext != ".pdf":
    ...
    elif materials["type"] == "video":
        video_url = (data.get("video_url") or "").strip()
        if not video_url:
            return jsonify(status="error", message="video_url required"), 400
        materials["video_url"] = video_url
    else:
        materials["type"] = "external"
        content_url = (data.get("content_url") or data.get("external_url") or "").strip()
        if not content_url:
            return jsonify(status="error", message="content_url required"), 400
        materials["content_url"] = content_url
...
def api_v1_get_quiz(course_id: str):
    ...
    course = next((c for c in courses if c.get("id") == course_id), None)
    if not course:
        return jsonify(quiz=None, message="Course not found"), 404
    quiz = course.get("quiz") or load_quizzes().get(course_id, {})
    ...
    return jsonify(quiz={
        "course_id": course_id,
        "pass_score": int(quiz.get("pass_score") or quiz.get("passing_score") or 70),
...
def api_v1_create_quiz(course_id: str):
    ...
    pass_score_raw = data.get("pass_score") or data.get("passing_score") or 70
    ...
    for i, q in enumerate(questions):
        text = (q.get("text") or q.get("question") or "").strip()
        options = q.get("options") or []
        correct_index = q.get("correct_index", q.get("correct"))
        try:
    ...
    # Keep legacy quizzes file in sync for backward compatibility
    quizzes = load_quizzes()
    quizzes[course_id] = course["quiz"]
    save_quizzes(quizzes)

    return jsonify(status="success", message="Quiz saved"), 201


def grade_course_quiz_submission(course_id: str, student: str, answers: dict):
    courses = load_courses()
    course = next((c for c in courses if c.get("id") == course_id), None)
    if not course:
    ...
    if student not in enrollments.get(course_id, {}) and student not in course.get("students", []):
        return {"status": "error", "message": "Not enrolled"}, 403

    quiz = course.get("quiz") or load_quizzes().get(course_id)
    if not quiz or not quiz.get("questions"):
        return {"status": "error", "message": "No quiz for this course"}, 404

    questions = quiz.get("questions", [])
    total = len(questions)
    if total == 0:
        return {"status": "error", "message": "Quiz has no questions"}, 500
    ...
    for q in questions:
        ...
        correct_index = q.get("correct_index", q.get("correct"))
        is_correct = user_answer is not None and int(user_answer) == int(correct_index)
        if is_correct:
            correct += 1
        results.append({
            "question_id": qid,
...
def api_v1_submit_quiz(course_id: str):
    ...
    result, code = grade_course_quiz_submission(course_id, student, answers)
