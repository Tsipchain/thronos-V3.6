...
class S:
    ...
    # admission hooks
    def resolve_wallet_pledge_state(self, addr):
        return {'active': addr in self.pledged}
...
def test_inactive_non_pledged_rejected(monkeypatch, tmp_path):
    ...
    try:
        a.require_active_thr_address('NOPE')
        assert False
...
def test_migrated_old_blocked_only_after_completed(monkeypatch, tmp_path):
    s = S(); _setup(monkeypatch, tmp_path, s)
    (tmp_path/'m.json').write_text('{"OLD":{"old_address":"OLD","new_v1_address":"NEW","status":"pending"}}')
...
def test_migrated_new_admitted_only_after_completed_or_repaired(monkeypatch, tmp_path):
    s = S(); _setup(monkeypatch, tmp_path, s)
    (tmp_path/'m.json').write_text('{"OLD":{"old_address":"OLD","new_v1_address":"NEW","status":"failed"}}')
    try:
        a.require_active_thr_address('NEW'); assert False
    except a.AdmissionError:
        pass
    (tmp_path/'m.json').write_text('{"OLD":{"old_address":"OLD","new_v1_address":"NEW","status":"repaired"}}')
...
def test_client_supplied_new_address_not_trusted(monkeypatch, tmp_path):
    s = S(); _setup(monkeypatch, tmp_path, s)
    pub = '02' + '11' * 32
    rec = m.migrate_legacy_address('OLD', 'ok', pub)
    assert rec['new_v1_address'] == m.derive_thronos_address(pub)
